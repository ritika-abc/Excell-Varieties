(function($) {
    ...
    // Date Picker
    if ($(".garlon-datepicker").length) {
        $(".garlon-datepicker").each(function() {
            $(this).datepicker();
        });
    }
    ...
    //accrodion
    if ($(".garlon-accrodion").length) {
        var accrodionGrp = $(".garlon-accrodion");
    ...
    function thmOwlInit() {
        // owl slider
        let garlonowlCarousel = $(".garlon-owl__carousel");
        if (garlonowlCarousel.length) {
            garlonowlCarousel.each(function() {
                let elm = $(this);
        ...
        let garlonowlCarouselNav = $(".garlon-owl__carousel--custom-nav");
        if (garlonowlCarouselNav.length) {
            garlonowlCarouselNav.each(function() {
                let elm = $(this);
        ...
        let garlonowlCarouselWithFilter = $(".garlon-owl__carousel--filter");
        if (garlonowlCarouselWithFilter.length) {
            garlonowlCarouselWithFilter.each(function() {
                let elm = $(this);
                let options = elm.data("owl-options");
    ...
    function garlonSlickInit() {
        // slick slider
        let garlonslickCarousel = $(".garlon-slick__carousel");
        if (garlonslickCarousel.length) {
            garlonslickCarousel.each(function() {
                let elm = $(this);
                let options = elm.data("slick-options");
                let garlonslickCarousel = elm.slick(
                    "object" === typeof options ? options : JSON.parse(options)
                );
            });
        }
        let garlonslickCarouselCounter = $(".garlon-slick__custome-counter");
        if (garlonslickCarouselCounter.length) {
            garlonslickCarouselCounter.each(function() {
                let elm = $(this);
                let options = elm.data("slick-options");
                let currentSlide;
                let slidesCount;
                let sliderCounter = document.createElement('div');
                sliderCounter.classList.add('garlon-slick__counter');

                let updateSliderCounter = function(slick, currentIndex) {
                    currentSlide = slick.slickCurrentSlide() + 1;
                    slidesCount = slick.slideCount;
                    $(sliderCounter).html('<span class="garlon-slick__counter__active">' + currentSlide + '</span>' + '' + '<span>' + slidesCount + '</span>')
                };
    ...
    /*-- Hover Tilt --*/
    let garlonTiltElm = $('.garlon-tilt');
    if (garlonTiltElm.length) {
        garlonTiltElm.each(function() {
            let self = $(this);
            let options = self.data('tilt-options');
            let garlonTilt = self.tilt(
                "object" === typeof options ? options : JSON.parse(options)
            );
        });
    }
    ...
    const link = document.querySelectorAll('.service-three__item');
    const linkHoverReveal = document.querySelectorAll('.service-three__item__box');
    ...
    $('.team-card__btn, .team-three-card__btn').on('click', function(e) {
        e.preventDefault(); // prevent link behavior

        const social = $(this).closest('.team-card, .team-three-card').find('.team-card__social, .team-three-card__social');
    ...
    $('.team-card-two').on('click', function(e) {
        e.preventDefault(); // prevent link behavior

        const social = $(this).closest('.team-card-two').find('.team-card-two__hover__social');
